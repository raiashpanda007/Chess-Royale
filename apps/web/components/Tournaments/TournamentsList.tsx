...
function TournamentsList({
  ...
  numberOfPlayers,
    ...
    const joinContest = async (tournamentId:string) =>{
        try {
            const response = await axios.put(`https://web.chesssroyale.games/api/tournament/join/userRequest`, {tournamentId});
  ...
  return (
    <div className="w-full h-36 rounded-3xl flex items-center hover:border hover:bg-gray-900 animate-in cursor-pointer font-poppins" >
      <div className="w-1/6 flex items-centter justify-center ">
        <img src={logo ? logo : '/tournamentlogo.jpg'} alt="logo" className="w-12 h-12 sm:w-20 sm:h-20 rounded-full" />
      </div>
